...
> extends React.Component<IDraggableGridProps<DataType>, State> {
    dragStarted: boolean;
    ...
    onHandMove = (nativeEvent: any, gestureState: any) => {
        ...
        if (activeItemIndex !== closestItemIndex) {
            const closestOrder = this.orderMap[this.items[closestItemIndex].key]
                .order;
    ...
    setActiveBlock = (itemIndex: number, item: DataType) => {
    ...
    getDefaultDragStartAnimation = () => {
        return {
            transform: [{ scale: this.state.dragStartAnimatedValue }],
            shadowColor: '#000000',
            shadowOpacity: 0.2,
            shadowRadius: 6,
    ...
    addItem = (item: DataType, index: number) => {
    ...
    removeItem = (item: IItem<DataType>) => {
